...
if(!function_exists('datatablePath')){
    function datatablePath(){
        return app_path() . '/DataTables/';
    }
}

if(!function_exists('getStubContent')){
    function getStubContent($stubName, $path = ''){
        $stub = stubPath($path) . "{$stubName}.stub";
        return file_get_contents($stub);
    }
}

if(!function_exists('isLanguageActive')){
    function isLanguageActive($languageExcerpt){
        return $languageExcerpt == \Illuminate\Support\Facades\App::getLocale();
    }
}
...
if(!function_exists('isPersian')){
    function isPersian(){
        return \Illuminate\Support\Facades\App::getLocale() == 'fa';
    }
}

if(!function_exists('getLocaleDirection')){
    function getLocaleDirection(){
        return \Illuminate\Support\Facades\Config::get('laravellocalization.supportedLocales.' . \Illuminate\Support\Facades\App::getLocale())['direction'];
    }
}

if(!function_exists('createLanguageFile')){
    function createLanguageFile($file, $excerpt){
        $languageDirectory = lang_path($excerpt);
        if( !File::exists($languageDirectory) )
            File::makeDirectory($languageDirectory);

        $dashboardDirectory = $languageDirectory .'/dashboard/';
        if( !File::exists( $dashboardDirectory ) )
            File::copyDirectory(lang_path('base/dashboard'), $dashboardDirectory);

        $path = $dashboardDirectory . $file;
        if( !File::exists($path) )
            File::copy(lang_path('base/dashboard/' . $file), lang_path($excerpt) . '/dashboard/' . $file);

        $dashboardFile = $languageDirectory . '/dashboard.php';
        if( !File::exists($dashboardFile) )
            File::copy(lang_path('base/dashboard.php'), $dashboardFile);

        return $path;
    }
}

if(!function_exists('convertConfigArrayToConfigFile')){
    function convertConfigArrayToConfigFile(array $array, $depth = 1){
        $structure = "";
        foreach ($array as $key => $value){
            if( !is_array($value) )
                $structure .= str_repeat("\t", $depth) . "'{$key}' => '{$value}',\n";
            else {
                $structure .= str_repeat("\t", $depth) . "'{$key}' => [\n".convertConfigArrayToConfigFile($value, $depth + 1).  str_repeat("\t", $depth) . "],\n";
            }
        }
